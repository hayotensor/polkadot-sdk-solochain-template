...
use crate::{
  ...
  SubnetNodePenalties, PreliminarySubnetData,
...
#[test]
fn test_activate_subnet_max_subnet_mem_err() {
  new_test_ext().execute_with(|| {
    let cost = Network::get_subnet_initialization_cost(0);
    let _ = Balances::deposit_creating(&account(0), cost+1000);
...
fn test_add_subnet_node_remove_readd_err() {
  new_test_ext().execute_with(|| {
    ...
    // Add block so SubnetAccount BTreeMap doesn't default to ``0``
...
fn test_add_subnet_node() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(Network::total_subnet_stake(subnet_id), amount_staked);
    assert_eq!(TotalSubnetNodes::<Test>::get(subnet_id), n_peers);
    ...
    for n in 0..n_peers {
      ...
      let subnet_accounts = SubnetAccount::<Test>::get(subnet_id);
      let block_initialized = subnet_accounts.get(&account(n)).unwrap();
      assert_eq!(block_initialized, &block_number);
  
      let account_subnets = AccountSubnets::<Test>::get(account(n));
      assert_eq!(account_subnets.len(), 1);
      assert_eq!(account_subnets[0], subnet_id);
...
fn test_claim_stake_unbondings() {
  new_test_ext().execute_with(|| {
    ...
    let starting_balance = Balances::free_balance(&account(1));
    ...
    let after_stake_balance = Balances::free_balance(&account(1));
    ...
    let unbondings: BTreeMap<u64, u128> = SubnetStakeUnbondingLedger::<Test>::get(account(1), subnet_id);
...
fn test_remove_to_stake_max_unlockings_reached_err() {
  new_test_ext().execute_with(|| {
    ...
    let starting_balance = Balances::free_balance(&account(0));
    // assert_eq!(starting_balance, deposit_amount);
    ...
    for n in 0..max_unlockings+1 {
      ...
      if n+1 > max_unlockings {
        ...
        let unbondings: BTreeMap<u64, u128> = SubnetStakeUnbondingLedger::<Test>::get(account(0), subnet_id);
...
fn test_remove_subnet_node() {
  new_test_ext().execute_with(|| {
    ...
    let node_set = SubnetNodesClasses::<Test>::get(subnet_id, SubnetNodeClass::Idle);
    assert_eq!(node_set.len(), (n_peers - remove_n_peers) as usize);

    assert_eq!(Network::total_stake(), amount_staked);
    assert_eq!(Network::total_subnet_stake(subnet_id), amount_staked);
    assert_eq!(TotalSubnetNodes::<Test>::get(subnet_id), n_peers - remove_n_peers);
    ...
    for n in 0..remove_n_peers {
      ...
      let account_subnets = AccountSubnets::<Test>::get(account(n));
      assert_eq!(account_subnets.len(), 0);
...
///
///
/// Rewards triggered by validator
///
///
///
///
...
#[test]
fn test_reward_subnet_invalid_epoch() {
  new_test_ext().execute_with(|| {
    let subnet_path: Vec<u8> = "petals-team/StableBeluga2".into();
...
#[test]
fn test_reward_subnet_invalid_subnet_id() {
  new_test_ext().execute_with(|| {
    let subnet_path: Vec<u8> = "petals-team/StableBeluga2".into();
...
#[test]
fn test_reward_subnet_rewards_complete() {
  new_test_ext().execute_with(|| {
    let subnet_path: Vec<u8> = "petals-team/StableBeluga2".into();
    ...
    assert_err!(
      ...
      Error::<Test>::SubnetRewardsSubmissionComplete
    );
  });
}

#[test]
fn test_reward_subnet_no_submission() {
  new_test_ext().execute_with(|| {
    let subnet_path: Vec<u8> = "petals-team/StableBeluga2".into();
...
#[test]
fn test_reward_subnet_no_submission_or_validator() {
  new_test_ext().execute_with(|| {
    let subnet_path: Vec<u8> = "petals-team/StableBeluga2".into();
